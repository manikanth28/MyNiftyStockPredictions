...
- **API and monitoring** (`api-and-monitoring`) - Added a read-only cached monitoring snapshot, `/api/monitoring` JSON endpoint, `/monitoring` operator page, centralized source success/failure summaries, freshness alerts, refresh/run visibility, retry-policy display, and structured monitoring events.
...
- **Build portfolio page** (`portfolio-wallet-page`) - Added `/portfolio` with navigation, persistent header holdings-value summary, starting cash reset, available cash, blocked entry value, live portfolio value, total equity, realized/unrealized P&L, open positions, closed trades, and recommendation ticket workflow.
- **Reconcile wallet outcomes** (`portfolio-wallet-evaluator`) - Added live-price target/stop auto-exits, archive-outcome reconciliation for target/stop hits, partial manual exits with selected quantities, realized P&L cash return, and month-scale performance review.
...
- **Shared wallet background trader** (`shared-wallet-background-trader`) - Added shared file-backed paper-wallet storage/API for UI and bot, migrated screen buy/sell flows to the shared wallet, added the requested portfolio holdings grid and bot report list, added deterministic background buy/sell/report automation, Markdown/JSON report files, Windows notification fallback, npm scripts, and a bot launcher batch file.
